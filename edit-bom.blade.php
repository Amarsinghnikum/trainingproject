...
<script>
  ...
  if(val == 2){
    ...
    add_incr = i+1;
    add_decr = i-1;
      if(document.getElementById('attributes_value_div_bom_'+i).style.display == "none"){
        document.getElementById('newBox_3_'+i).style.display = "none";
        document.getElementById('attributes_value_div_bom_'+i).style.display = "block";
...
function del_more_record_pdf(name,id){
  if(name == 'attributes'){         
      if(document.getElementById('attributes_value_div_bom_'+id).style.display == "block"){
      decr1 = id-1;
      document.getElementById('suppl_code'+id).value="";
      document.getElementById('item_name'+id).value="";
      document.getElementById('qty'+id).value="";
      document.getElementById('unit'+id).value="";
      document.getElementById('attributes_value_div_bom_'+id).style.display = "none";
    ...
    <div class="container-fluid">
        <div class="row clearfix">
            ...
            <div class="col-lg-12 col-md-12">
                <div class="card">
                   <ul class="nav nav-tabs">
                        <li class="nav-item"><a class="nav-link active" data-toggle="tab" href="#about">Update Bom</a></li>                       
                    </ul>
                    <div class="tab-content">
                        
                        <div class="tab-pane body active" id="about">
                                    <form action="{{url('/update-bom/'.$bomitems->id)}}" method="post">
                                        @csrf
                                        @method('PUT')
                                              ...
                                              <div class="row">
                                                   <div class="col-sm-3">
                                                        <div class="form-group">
                                                          <label for="inputName"><b>BOM ITEM SUPPL CODE</b></label>
                                                          <input type="text" name="bom_supplcode" class="form-control" value="{{$bomitemss->bom_supplcode}}">
                                                            @if (!empty($errors))
                                                            <div style="color:red">{{ $errors->first('bom_supplcode') }}</div>
                                                            @endif
                                                        </div>
                                                    </div>
                                                    <div class="col-sm-3">
                                                        <div class="form-group">
                                                          <label for="inputName"><b>BOM ITEM NAME</b></label>
                                                          <input type="text" name="bom_itemname" class="form-control" value="{{$bomitemss->bom_itemname}}">
                                                        @if (!empty($errors))
                                                            <div style="color:red">{{ $errors->first('bom_itemname') }}</div>
                                                        @endif
                                                        </div>
                                                  </div>
                                              </div>
                                          ...
                                          <div class="card card-primary my-3">
                                             <div class="card-body" id="demo">
                                                <div>
                                                    <?php for($j=1;$j<=5;$j++){ ?>
                                                    <div class="row">
                                                    <div class="col-md-12" id="attributes_value_div_bom_<?=$j?>" <?php if($j==1)
                                                    { echo 'style="display:block;"'; }else{ ?>style="display:none;" <?php } ?>>
                                                    <div class="row">

                                                    <div class="col-md-3">
                                                    <label for=""><b>SUPPL CODE</b></label>
                                                        <select class="selectpicker" name="suppl_code<?=$j?>" id="suppl_code<?=$j?>" multiple>
                                                        <option value="0">Select State Name</option>
                                                        @foreach($supplcode as $key=>$val)
                                                        <option value="{{$val->suppl_code}}">{{$val->suppl_code}}</option>
                                                        @endforeach
                                                        </select>
                                                    </div>
                                                    ...
                                                    <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label for="inputName"><b>ITEM NAME</b></label>
                                                        <input type="text" class="form-control" name="item_name<?=$j?>" id="item_name<?=$j?>" multiple>
                                                    </div>
                                                    </div>

                                                    <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label for="inputName"><b>QTY</b></label>
                                                        <input type="text" class="form-control" name="qty<?=$j?>" id="qty<?=$j?>" multiple>
                                                    </div>
                                                    </div>

                                                    <div class="col-md-3">
                                                    <div class="form-group">
                                                        <label for="inputName"><b>UNIT</b></label>
                                                        <input type="text" class="form-control" name="unit<?=$j?>" id="unit<?=$j?>" multiple>
                                                    </div>
                                                    </div>
                                            ...
                                            <?php $j=1;
                                            foreach($bomitem as $val){ 
                                                $j = $val->id;
                                            ?>   
                                            @if($val->suppl_code!='')
                                            <div id="attributes_value_div_bom_<?=$val->id?>" >
                                             <div class="row">
                                                    <div class="col-md-3">
                                                    <label for=""><b>SUPPL CODE</b></label>
                                                        <select class="selectpicker" name="suppl_code<?=$j?>" id="suppl_code<?=$j?>" value="{{$val->suppl_code}}" multiple>
                                                        <option value="0">Select State Name</option>
                                                        <option value="{{$val->suppl_code}}" {{$val->suppl_code?'selected':''}}>{{$val->suppl_code}}</option>
                                                        </select>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="inputName"><b>ITEM NAME</b></label>
                                                        <input type="text" class="form-control" value="{{$val->item_name}}" name="item_name<?=$j?>" id="item_name<?=$j?>"  multiple>
                                                    </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="inputName"><b>QTY</b></label>
                                                        <input type="text" class="form-control" value="{{$val->qty}}" name="qty<?=$j?>" id="qty<?=$j?>"  multiple>
                                                    </div>
                                                    </div>
                                                    ...
                                                    <div class="form-group">
                                                        <label for="inputName"><b>UNIT</b></label>
                                                        <input type="text" class="form-control" value="{{$val->unit}}" name="unit<?=$j?>" id="unit<?=$j?>" value="{{$val->unit}}" multiple>
                                                    </div>
                                                    </div>
                                                    ...
                                                    <?php if($j!=1){ ?>
                                                        <div class="col-sm-4" id="cancel_3_<?=$j?>" style="display:block;">
                                                        <button type="button" onclick="return del_more_record_pdf1('attributes','<?=$j?>');" class="btn btn-danger mt-4 d-inline-block">
                                                         Remove
                                                        </button>
                                                        </div>            
                                                    <?php }?>
...
<!-- *********************************************************************************************************************************************** -->
                                            <div class="col-sm-8">
                                                <div class="form-group">
                                                    <label for="inputName"><b>REMARK</b></label>
                                                    <textarea name="remark" class="form-control">{{$bomitemss->remark}}</textarea>
                                                    @if (!empty($errors))
